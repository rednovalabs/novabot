...
module.exports = (robot) ->
  ...
  robot.logger.info("Posting commit links to Github repos: " + process.env.HUBOT_GITHUB_REPOS)
  repos = JSON.parse(process.env.HUBOT_GITHUB_REPOS)
  ...
  robot.hear ///
      ...
      ///i, (msg) ->
    if process.env.HUBOT_GITHUB_REPOS && process.env.HUBOT_GITHUB_TOKEN
      if !(msg.message.text.match(/commit\//))
        commit_sha = msg.match[2]
        if msg.match[1]?
          bot_github_repo = github.qualified_repo msg.match[1]
        else
          bot_github_repo = github.qualified_repo repos[msg.message.room]
        issue_title = ""
      ...
      msg.send "Hey! You need to set HUBOT_GITHUB_REPOS and HUBOT_GITHUB_TOKEN before I can link to that commit."
